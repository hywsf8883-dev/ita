...
client.on('interactionCreate', async (i) => {
  ...
  if (i.isStringSelectMenu() && i.customId === 'delete') { const h = db.hosts.find(x => x.id === i.values[0]); if (!h) return i.update(ui('## الهوست غير موجود')); await deleteServer(h.pteroId); db.hosts = db.hosts.filter(x => x.id !== h.id); write(db); return i.update(ui('## تم حذف الهوست\nتم حذف الهوست من Pterodactyl بنجاح.')); }
