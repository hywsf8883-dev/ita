...
const ui = (message, extra = []) => ({ flags: MessageFlags.IsComponentsV2, components: [new ContainerBuilder().addTextDisplayComponents(new TextDisplayBuilder().setContent(message)), ...extra] });
...
client.on('messageCreate', async (m) => {
  ...
  try {
    if (command === '.cra') { const discordId = args.shift()?.match(/^<@!?([0-9]+)>$/)?.[1]; const [email, password] = args; if (!discordId || !email || !password) return m.reply(ui('الاستخدام: `.cra @user email@example.com password`')); if (db.accounts.some(x => x.email === email)) return m.reply(ui('الحساب موجود بالفعل.')); const user = await createPanelUser(email, password); db.accounts.push({ email, discordId, pteroId: user.id }); write(db); return m.reply(ui(`تم إنشاء حساب Pterodactyl وربطه بـ <@${discordId}>\n**Email:** ${email}`)); }
...
client.on('interactionCreate', async (i) => {
  ...
  if (i.isButton() && i.customId.startsWith('delete-all:')) { const key = i.customId.slice(12); const hosts = db.hosts.filter(h => key === 'all' || h.discordId === key); for (const h of hosts) await deleteServer(h.pteroId); db.hosts = db.hosts.filter(h => !hosts.includes(h)); write(db); return i.update(ui(`تم حذف ${hosts.length} هوست.`)); }
